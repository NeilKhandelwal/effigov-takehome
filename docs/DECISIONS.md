...
2026-08-30 | SQLAlchemy Core, not the ORM, under Alembic | migrations were the non-negotiable; Core keeps every query a visible statement at the call site, which is how this backend reads | a session/identity map, and lazy relationship loading we would otherwise get for free
2026-08-30 | `DATABASE_URL`, defaulting to SQLite; Postgres in compose and CI | the local three-terminal path has to stay infra-free for a new contributor, and the deployed path has to be the engine we actually test against | two engines to keep green — every schema change now has to survive both CI jobs
2026-08-30 | notes are `case_events` rows, `Case.notes` derived; `PATCH notes` is a 422 | add_note was a GET then a PATCH of the whole string, so two writers in one second lost a note, and the log recorded a rewrite rather than the note | the agent's add_note and the dashboard textarea break until the follow-up PR moves them — the one intentional break
2026-08-30 | `city_id` written on every row now, reads not scoped yet | writing the column later means migrating live data; scoping reads later is a query change | a column that does nothing yet, and a scope that is still unenforced
2026-08-30 | `?since=` is inclusive (`>=`), not strictly after | timestamps are second-resolution, so a strict cursor drops rows written in its own second and never returns them; clients already tolerate duplicates because every WS frame is a refetch | clients see the boundary row again on each poll
2026-08-30 | a pre-migration database is refused at startup, not upgraded | SQLite DDL is not transactional, so a failed in-place upgrade leaves a half-migrated file and a different error on the next boot; the ids and the notes column changed shape, so there was nothing to upgrade from anyway | anyone holding an old cases.db has to move it aside and reseed
2026-08-30 | the dashboard's notes card is append-only, built from the case's `note` events rather than the derived `Case.notes` string | the events carry the timestamp and the source, so staff can see which notes came off a call and when; a textarea over a joined string can only offer a rewrite, which is the write the backend now refuses | staff can no longer fix a typo in their own note — correcting one means adding another
2026-08-30 | `case_events.actor` from the `X-Actor` header, nullable and unauthenticated: `source` keeps saying which system, `actor` says which person | replacing `staff` with a name would have lost the system/person distinction the agent's `voice` rows depend on, and a nullable column is honest about voice calls and pre-0003 rows rather than backfilling a guess | the known gap: the API trusts the header, so anything that can reach the backend can name itself anyone — the dashboard session is the only thing behind the name until backend auth lands, and nothing is enforced at the API yet
